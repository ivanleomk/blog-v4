...
<Latex formula="1 + 2 + \dots + n = \frac{n(n+1)}{2}" />
...
- `last_x,last_num, last_den` : These are all stored in a huge `(N_Layers, 4,N_embeddings)` array and simply store the previous computations for each layer.
  ...
  - `last_x`: <Latex math = "x_{t-1}" />
  - `last_num`:<Latex math = "wkv_{t-1}" /> numerator
  - `last_den`:<Latex math = "wkv_{t-1}" /> denominator

- `decay, bonus, mix_k, mix_v, mix_r, Wk, Wv, Wr, Wout`
  - `decay` : `w` parameter can be treated as <Latex math = "e^{-\text{decay}}" />
  - `bonus` : This is equivalent to the `u` parameter above
  - `mix_k`,`mix_r` and `mix_v` are equal to <Latex math = "\mu_k,\mu_r,\mu_t" />
  - `Wk,Wv,Wr and wout` are equivalent to `W_k,W_v,W_r` and `W_o` above
- Note here that <Latex math = "\sigma" /> simply represents the sigmoid activation function.

In the code above, it might be a little bit difficult to visualise the dimensions so

- (1024,1024) : <Latex formula= "W_r, W_k, W_v" />
- (1024, ): <Latex formula= "\mu_r,x_t,x_{t-1},\mu_k,\mu_v" /> , decay, bonus

This means that essentially <Latex formula= "r_t,k_t" /> and <Latex formula= "v_t" /> are all going to be of the dimension `(1024,)` and the final output of this function will be of `(1024,)`.
...
<Latex formula="wkv_t=\frac{\sum_{i=1}^{t-1}e^{-(t-1-i)w+k_i}v_{i} + e^{\mu+k_t}v_t}{\sum_{i=1}^{t-1}e^{-(t-1-i)w+k_i} + e^{\mu+k_t}}" />
...
<Latex formula="\sum_{i=1}^{t-1}e^{-(t-1-i)w+k_i}v_{i}" />

Let's also designate the sum of all the elements defined in the sum as <Latex formula= "\alpha_{t-1}" /> where we are summing elements from 1 to <Latex math = "t-1" />. How do we get from <Latex formula="\alpha_{t-1}" /> to <Latex formula="\alpha_{t}" />?

If we look at the term <Latex formula= "\alpha_{t-1}"/>, we can see that the final value of the summation is going to be

<Latex formula="e^{-(t-1-(t-1))w+k_i} = e^{k_{t-i}}" />

Well, what about the second last value of the summation then? it's going to be

<Latex formula="e^{-(t-1-(t-2))w+k_{t-2}} = e^{-w + k_{t-i}}" />

This means that we can therefore link <Latex formula= "\alpha_{t-1}" /> with <Latex formula= "\alpha_{t}" /> by doing

<Latex formula="\sum_{i=1}^{t-1}e^{-(t-1-i)w+k_i}v_{i} = \sum_{i=1}^{t-2}e^{-(t-1-i)w+k_i}v_{i} + e^{k_{t-1}}" />

We can perform a similar substituition to get <Latex formula= "\beta_{t} =\beta_{t-1}+ e^{k_{t-1}}" />.
...
We can then rewrite our equation for <Latex formula= "wkv_t" /> as

<Latex formula="wkv_{t} = \frac{\alpha_{t-1} + e^{\text{bonus} + k_{t}}v_t }{\beta_{t-1} + e^{\text{bonus} + k_{t}}}" />
...
It's significantly easier to understand. **Note that all these parameters are learned parameters with the exception of the <Latex math = "x_{t-1}" /> values**.
...
- It's interesting here to note that
  - <Latex math = "w_k" /> has a dimensionality of <Latex math = "[4096,1024]" />
  - <Latex math = "w_v" /> has a dimensionality of <Latex math = "[1024,4096]" />
